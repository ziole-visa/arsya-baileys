...
import { generateMessageIDV2, unixTimestampSeconds, createMetadataCache } from '../Utils/index.js';
...
export const makeGroupsSocket = (config) => {
    ...
    /** arsya-baileys: built-in group metadata TTL cache + coalescing */
    const metadataCache = createMetadataCache(config);
    const groupMetadata = async (jid) => {
        ...
        return metadataCache.getOrFetch(jid, fetchLive);
    };
    const groupMetadataFresh = async (jid) => {
        const result = await groupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }]);
        return extractGroupMetadata(result);
    };
    // Invalidate cache when group events arrive
    ...
    ev.on('group-participants.update', (update) => {
        if (!metadataCache || !update?.id) return;
    ...
    return {
        ...
        groupUpdateDescription: async (jid, description) => {
            const metadata = await groupMetadataFresh(jid);
