...
export const makeMessagesRecvSocket = (config) => {
    ...
    // --- arsya-baileys: auto decrypt retry options ---
    const decryptRetryOpts = resolveDecryptRetryOptions(config);
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            await messageMutex.mutex(async () => {
                // --- arsya-baileys: auto-retry decrypt on transient failures ---
                const maxDecryptAttempts = decryptRetryOpts.maxAttempts;
                for (let decryptAttempt = 1; decryptAttempt <= maxDecryptAttempts; decryptAttempt++) {
                    let hardError = null;
                    try {
                        await decrypt();
                    }
                    catch (decryptErr) {
                        hardError = decryptErr;
                    }
                    const failedStub = msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer';
                    if (!hardError && !failedStub) {
                        break;
                    }
                    const isLast = decryptAttempt >= maxDecryptAttempts;
                    if (isLast) {
                        if (hardError) {
                            logger.error({
                                err: hardError,
                                ...
                                msgId: msg.key?.id
                            }, '[arsya-baileys] decrypt failed after retries');
                            msg.messageStubType = proto.WebMessageInfo.StubType.CIPHERTEXT;
                            msg.messageStubParameters = [hardError?.message?.toString() || String(hardError)];
                        }
                        break;
                    }
                    logger.warn({
                        err: hardError || msg.messageStubParameters?.[0],
                        attempt: decryptAttempt,
                        maxAttempts: maxDecryptAttempts,
                        msgId: msg.key?.id
                    }, '[arsya-baileys] decrypt failed, retrying');
                    // clear stub so a successful retry is not treated as ciphertext
                    if (!hardError) {
                        delete msg.messageStubType;
                        delete msg.messageStubParameters;
                    }
                    await delay(decryptRetryOpts.delayMs * decryptAttempt);
                }
