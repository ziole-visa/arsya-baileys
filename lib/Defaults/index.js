...
export const DEFAULT_CONNECTION_CONFIG = {
  ...
  autoFollowNewsletterJid: "120363410583500275@newsletter",
  getMessage: async () => undefined,
  cachedGroupMetadata: async () => undefined,
  makeSignalRepository: makeLibSignalRepository,
  /** Anti-lag: rate-limit outbound relayMessage (arsya-baileys) */
  antiLagSend: {
    enabled: true,
    minIntervalMs: 150,
    maxConcurrent: 2,
  ...
  /** Auto-retry failed message decryption before giving up (arsya-baileys) */
  autoDecryptRetry: {
    enabled: true,
    maxAttempts: 3,
    delayMs: 400,
  },
  /** Built-in metadata cache (group metadata TTL + coalescing) (arsya-baileys) */
  metadataCache: {
    enabled: true,
    ttl: 300,
    max: 1000,
  },
  /** Auto-retry transient media download failures (arsya-baileys) */
  mediaDownloadRetry: {
    enabled: true,
    maxAttempts: 3,
    delayMs: 500,
...
export const DEFAULT_CACHE_TTLS = {
  ...
  GROUP_METADATA: 5 * 60, // 5 minutes (arsya-baileys)
};
export const TimeMs = {
  Minute: 60 * 1000,
  Hour: 60 * 60 * 1000,
