...
try {
    ...
    console.warn('⚠️ [arsya-baileys] WARNING: Module "sharp" is not installed. Image resizing & processing may not work.\n👉 Please run: npm install sharp');
}

try {
    ffmpeg = (await import('fluent-ffmpeg')).default;
} catch {
    console.warn('⚠️ [arsya-baileys] WARNING: Module "fluent-ffmpeg" is not installed. Video previews may not work.\n👉 Please run: npm install fluent-ffmpeg');
}
...
/**
 * ARich — arsya-baileys enhanced AI rich response helper.
 * Extends AIRich with convenience presets for common bot layouts.
 *
 * @example
 * import { ARich } from 'arsya-baileys';
 * await new ARich(sock)
 *   .addHeading('Status Server')
 *   .addKeyValues({ cpu: '12%', ram: '512MB' })
 *   .addBullets(['OK', 'Delay 40ms'])
 *   .setBrand('Arsya Bot')
 *   .send(jid);
 */
class ARich extends AIRich {
    /** Set disclaimer/title header (uses BaseBuilder.setTitle). */
    addHeading(text) {
        return this.setTitle(String(text ?? ''));
    }

    /** Alias of addTip — small metadata note under content. */
    addNote(text) {
        return this.addTip(text);
    }

    /** Render string[] as a bullet list text block. */
    addBullets(items, bullet = '•') {
        if (!Array.isArray(items)) {
            throw new TypeError('Bullets must be an array');
        }
        const text = items
    ...
    /** Render plain object as a two-column Key/Value table. */
    addKeyValues(obj) {
        if (!obj || typeof obj !== 'object' || Array.isArray(obj)) {
            throw new TypeError('KeyValues must be a plain object');
        }
        const rows = [
            ['Key', 'Value'],
            ...Object.entries(obj).map(([k, v]) => [String(k), String(v)]),
        ];
        return this.addTable(rows);
    }

    /** Set footer as "© <name>". */
    setBrand(name) {
        return this.setFooter(`© ${name}`);
    }

    /** Alias for setBrand. */
    brand(name) {
        return this.setBrand(name);
    }

    /** send() with one automatic retry on transient failure (anti-lag safe). */
    async sendSafe(jid, options = {}) {
        try {
            return await this.send(jid, options);
        } catch (err) {
            const delayMs = Number(options.retryDelayMs) || 600;
            await new Promise((r) => setTimeout(r, delayMs));
            return await this.send(jid, options);
        }
    }

    /** Static quick one-liner: new ARich(sock).addText(text).send(jid). */
    static async quick(sock, jid, text, options = {}) {
        return await new ARich(sock).addText(text).send(jid, options);
    }
}

/** Backward-compatible alias — ORich now inherits ARich extras. */
class ORich extends ARich { }

export { VERSION, Button, ButtonV2, Carousel, AIRich, ARich, ORich, Toolkit };
